...
<div id="reviewsDefault" class="centerColumn rv_dts">
...
<?php  } ?>
    <div class="panel-default head-wrap">
		<div class="panel-heading mb30">
			<h4 class="panel-title"><?php echo NAVBAR_TITLE; ?></h4>
		</div>
	</div>
<?php
    	...
    	<div class="content">
        	<div class="row product-details-review">
            	<div class="smallProductImage back col-lg-3 col-md-3 col-sm-3 col-xs-12 img_ar">
					<?php echo '<a href="' . tep_href_link('product_info.php', 'products_id=' . 
					$product_info['products_id']) . '">' . tep_image(DIR_WS_IMAGES . 
					$product_info['products_image'], addslashes($product_info['products_name'])
					,'hspace="5" vspace="5"') . '</a>'; ?>
                </div>
                <div class="col-lg-9 col-md-9 col-sm-9 col-xs-12 product-review-default des_ar">
                    <h4><?php echo $products_name; ?></h4>
                    <p class="ds_ns">
					<?php echo tep_break_string(tep_output_string_protected($reviews['reviews_text']), 70, '-<br />'); ?></p>
                    <footer>
                    	<strong><?php echo TEXT_REVIEW_BY; ?></strong>
						<?php echo tep_output_string_protected($reviews['customers_name']); ?><br/>
                    	<strong><?php echo TEXT_REVIEW_DATE_ADDED; ?></strong>
						<?php echo tep_date_long($reviews['date_added']); ?><br/>
                        <?php echo sprintf(tep_image(DIR_WS_IMAGES . 'stars_' . $reviews['reviews_rating'] . '.gif', 
						sprintf(TEXT_OF_5_STARS,$reviews['reviews_rating'])), sprintf($reviews['reviews_rating'])); ?>
                    </footer>
                    <div class="wk-price mt10">
						<?php echo $products_price; ?>
                    </div>
					<div class="cart-buttons ckpc bkbc mt10">
                        <?php echo '<span><a href="' . tep_href_link('product_reviews_info.php', 'products_id=' . $product_info['products_id']. '&reviews_id=' . 
                        $reviews['reviews_id']) . '">Read Review</a></span>'; ?>
					</div>
                    <div class="cart-buttons ckpc bkbc mt10">
                        <?php echo tep_draw_button(IMAGE_BUTTON_WRITE_REVIEW, 'comment', 
                        tep_href_link('product_reviews_write.php', tep_get_all_get_params()), 'primary'); ?>
                    </div>
                    <div class="cart-buttons ckpc mt10">

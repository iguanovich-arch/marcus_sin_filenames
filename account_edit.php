...
<div id="accountEditDefault" class="centerColumn">
	...
	<div class="content shipping-outer nopad">
        ...
        <?php echo tep_draw_radio_field('gender', 'm', $male) . 
            '<label class="radioButtonLabel">' . MALE . '</label>' . tep_draw_radio_field
            ('gender', 'f', $female) . 
            '<label class="radioButtonLabel">' . FEMALE . '</label>' ;?>
            <span class="alert-text"><?php echo ENTRY_GENDER_TEXT; ?></span>
        <?php } ?>
        <br class="clearBoth">
        ...
        <div class="lable">
            ...
            <?php echo (tep_not_null(ENTRY_LAST_NAME_TEXT) ? '<span class="alert-text">' 
            . ENTRY_LAST_NAME_TEXT . '</span>': ''); ?>
        </div>
        ...
        <div class="lable">
            ...
            <?php echo (tep_not_null(ENTRY_DATE_OF_BIRTH_TEXT) ? 
            '<span class="alert-text">' . ENTRY_DATE_OF_BIRTH_TEXT . '</span>': ''); ?>
        </div>
        <?php echo tep_draw_input_field('dob', tep_date_short($account['customers_dob']), 'id="dob"') ;?>
        <script type="text/javascript">$('#dob').datepicker(
        {dateFormat: '<?php echo JQUERY_DATEPICKER_FORMAT; ?>', changeMonth: true, changeYear: true, 
        yearRange: '-100:+0'});</script>
        <?php } ?>
  
        <div class="lable">
            <?php echo ENTRY_EMAIL_ADDRESS; ?>
            <?php echo (tep_not_null(ENTRY_EMAIL_ADDRESS_TEXT) ? 
            '<span class="alert-text">' . ENTRY_EMAIL_ADDRESS_TEXT . '</span>': ''); ?>
        </div>
        <?php echo tep_draw_input_field('email_address', $account['customers_email_address']); ?>
        
        <div class="lable">
            <?php echo ENTRY_TELEPHONE_NUMBER; ?>
            <?php echo (tep_not_null(ENTRY_TELEPHONE_NUMBER_TEXT) ? 
            '<span class="alert-text">' . ENTRY_TELEPHONE_NUMBER_TEXT . '</span>': ''); ?>
        </div>
        <?php echo tep_draw_input_field('telephone', $account['customers_telephone']); ?>
  
        <div class="lable">
            <?php echo ENTRY_FAX_NUMBER; ?>
            <?php echo (tep_not_null(ENTRY_FAX_NUMBER_TEXT) ? 
            '<span class="alert-text">' . ENTRY_FAX_NUMBER_TEXT . '</span>': ''); ?>
        </div>
	...
	<div class="cart-buttons ck-btn">
		...
		<span class="cart-buttons">
			<?php echo tep_draw_button(IMAGE_BUTTON_BACK, 'triangle-1-w', 

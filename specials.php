...
?>
<?php echo $gridlist_tab=posc_gridlist_tab(FILENAME_SPECIALS); ?>
<div class="products-container grid-shop new-arrivals <?php echo $grid_list_class; ?>">
	<div class="row">
		...
		<?php
			$specials_query_raw = "select p.products_id, pd.products_name, pd.products_description, p.products_price, p.products_tax_class_id, p.products_model, p.products_weight, p.products_image, s.specials_new_products_price, f.featured_id, s.specials_id,
			m.manufacturers_name from " . TABLE_PRODUCTS . " p left join " . TABLE_FEATURED . " f on p.products_id = f.products_id left join " . TABLE_MANUFACTURERS . " m on (p.manufacturers_id = m.manufacturers_id)," . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_SPECIALS . " s  where p.products_status = '1' and s.products_id = p.products_id and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' and s.status = '1' order by s.specials_date_added DESC";
			$specials_split = new splitPageResults($specials_query_raw, MAX_DISPLAY_SPECIAL_PRODUCTS);

			if (($specials_split->number_of_rows > 0) && ((PREV_NEXT_BAR_LOCATION == '1') || (PREV_NEXT_BAR_LOCATION == '3'))) { ?>
			<div>
				<span><?php echo TEXT_RESULT_PAGE . ' <span class="pagination-style">' . $specials_split->display_links(MAX_DISPLAY_PAGE_LINKS, tep_get_all_get_params(array('page', 'info', 'x', 'y'))); ?></span></span>
			...
			if ($specials_split->number_of_rows > 0) {
				...
				while ($product_list = tep_db_fetch_array($specials_query)) {
...
<?php if (($specials_split->number_of_rows > 0) && ((PREV_NEXT_BAR_LOCATION == '2') || (PREV_NEXT_BAR_LOCATION == '3'))) { ?>
	<div class="pageresult_bottom">
		<div class="product-page-count">
			<div class="navSplitPagesResult">
				<?php echo $specials_split->display_count(TEXT_DISPLAY_NUMBER_OF_PRODUCTS_NEW); ?>
			</div>
		</div>
		<div class="navSplitPagesLinks pagination-style"><?php echo $specials_split->display_links(MAX_DISPLAY_PAGE_LINKS, tep_get_all_get_params(array('page', 'info', 'x', 'y'))); ?></div>
	</div>
<?php } ?>
<?php

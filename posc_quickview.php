...
<?php } else {
	...
	/*Reviews Query*/
	$review_query = tep_db_query("select products_id, reviews_rating from ".TABLE_REVIEWS." where products_id='$products_id'");
	$review_res = tep_db_fetch_array($review_query);
	if($review_res != NULL) {
		$rating_stars =  sprintf(tep_image(DIR_WS_IMAGES . 'stars_' . $review_res['reviews_rating'] . '.gif', sprintf(TEXT_OF_5_STARS,$review_res['reviews_rating'])), sprintf($review_res['reviews_rating']));
	}else {
	...
	<div class="ajxcart-info">
		...
		<div class="product-info-main">
			<div class="pinfo-right product-info">
				...
				<?php if(PRODUCT_QUICK_OVER_INFO != 0) { ?>
					<?php 

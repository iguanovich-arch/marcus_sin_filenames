      ...
      case 'update':
        ...
        if ($page_error == false) {
			...
			$language_idm='language_id_'.$languages[$i]['id'];
					  ...
					  $sql_data_array_pages = array('page_type'   => $_POST['page_type']);
      ...
      <tr><?php echo tep_draw_form('new_page', FILENAME_PAGE_MANAGER, (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . 'action=' . $form_action.$bIDif, 'post', 'enctype="multipart/form-data"'); if ($form_action == 'update') echo tep_draw_hidden_field('pages_id', $bID); ?>
		...
		<tr style="display:none">
            ...
            <td class="main"><?echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;'?><select name="page_type" size=1>
                                 <option value="1" <?php if($page_type == 1) { echo 'selected="selected"'; } ?>>1 - Shipping Information </option>
                                 <option value="2" <?php if($page_type == 2) { echo 'selected="selected"'; } ?>>2 - Terms & Condition </option>
 								 <option value="3" <?php if($page_type == 3) { echo 'selected="selected"'; } ?>>3 - Privacy Policy </option>
                                 <option value="4" <?php if($page_type == 4) { echo 'selected="selected"'; } ?>>4 -  Refund Policy </option>
                                 <option value="5" <?php if($page_type == 5) { echo 'selected="selected"'; } ?>>5 - Delivery Information </option>
                                 </select></td>
						  </tr>
...
<?php
    $pages_query_raw = "select p.pages_id, p.page_type, s.pages_title from " . TABLE_PAGES . " p LEFT JOIN " .TABLE_PAGES_DESCRIPTION . " s on p.pages_id = s.pages_id where s.language_id='" . (int)$languages_id . "' order by  s.pages_title";

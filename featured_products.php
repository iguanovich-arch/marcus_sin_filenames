...
*/
	...
	require(DIR_WS_LANGUAGES . $language . '/' . 'featured_products.php');
...
?>
<?php echo $gridlist_tab=posc_gridlist_tab('featured_products.php'); ?>
<div class="products-container grid-shop new-arrivals <?php echo $grid_list_class; ?>">
	<div class="row">
		...
		<?php
			...
			$featured_products_query_raw = "select p.products_id, pd.products_name, pd.products_description, p.products_image, p.products_price, p.products_tax_class_id, f.featured_id, s.specials_id, IF(s.status, s.specials_new_products_price, NULL) as 	specials_new_products_price, p.products_date_added, p.products_model, p.products_weight, m.manufacturers_name from " . TABLE_PRODUCTS . " p left join " . TABLE_MANUFACTURERS . " m on p.manufacturers_id = m.manufacturers_id left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on p.products_id = pd.products_id and pd.language_id = '" . $languages_id . "' left join " . TABLE_SPECIALS . " s on p.products_id = s.products_id
			left join " . TABLE_FEATURED . " f on p.products_id = f.products_id where p.products_status = '1' and f.status = '1' order by p.products_date_added DESC, pd.products_name";
			...
			<div>
				<span><?php echo TEXT_RESULT_PAGE . ' ' . $featured_products_split->display_links(MAX_DISPLAY_PAGE_LINKS, tep_get_all_get_params(array('page', 'info', 'x', 'y'))); ?></span>
			...
			<?php
				if ($featured_products_split->number_of_rows > 0) {
					...
					while ($product_list = tep_db_fetch_array($featured_query)){
...
<div class="pageresult_bottom">
	<div class="product-page-count">
		<div class="navSplitPagesResult">
			<?php echo $featured_products_split->display_count(TEXT_DISPLAY_NUMBER_OF_PRODUCTS_NEW); ?>
        </div>
	</div>
	<div class="navSplitPagesLinks pagination-style"><?php echo $featured_products_split->display_links(MAX_DISPLAY_PAGE_LINKS, tep_get_all_get_params(array('page', 'info', 'x', 'y'))); ?></div>
</div>
<?php  } ?>
<?php
